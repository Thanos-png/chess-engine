...
class ZobristHasher:
    ...
    def hash_castling(self, castling_rights: str) -> int:
    ...
    def hash_ep_square(self, ep_square: Optional[Tuple[int, int]], turn: str, pawns: Dict[Tuple[int, int], str]) -> int:
    ...
    def hash_turn(self, turn: str) -> int:
        ...
        return self.array[780] if turn == "white" else 0

    def __call__(self, pieces: Dict[Tuple[int, int], Tuple[str, str]], castling_rights: str, ep_square: Optional[Tuple[int, int]], turn: str, pawns: Dict[Tuple[int, int], str]) -> int:
        """Compute the full Zobrist hash."""
        return (
            self.hash_board(pieces)
            ^ self.hash_castling(castling_rights)
            ^ self.hash_ep_square(ep_square, turn, pawns)
            ^ self.hash_turn(turn)
        )
            ...
            with open(self.book_path, "rb") as book:
                ...
                while True:
                    ...
                    if entry_hash == zobrist_hash:
                        ...
                        yield weight, uci_move
        except FileNotFoundError:
            raise FileNotFoundError(f"Polyglot file not found: {self.book_path}")
        except Exception as e:
            raise RuntimeError(f"Error reading Polyglot file: {e}")
